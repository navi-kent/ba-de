...
爬蟲 (每天 4 次)          資料庫                  看板
┌──────────────┐          ┌────────────────┐      ┌─────────────────┐
│ news_rss.py  │──寫入───▶│ raw_posts      │◀─讀─│ dashboard/      │
│ ptt_scraper  │          │ processed_posts│      │ app.py :5001    │
└──────────────┘          │ scraper_runs   │      │                 │
                          │ wishes         │      │ /         首頁  │
(未來)                    └────────────────┘      │ /wish     許願池│
┌──────────────┐                                  └─────────────────┘
│ agent_       │──寫入───▶ processed_posts
│ analysis.py  │
└──────────────┘
```
...
2. 爬蟲將文章去重後寫入 `raw_posts`
3. 居民開啟 `http://127.0.0.1:5001` 即可瀏覽最新資訊
...
# 安裝套件
pip install flask feedparser pyyaml requests beautifulsoup4 python-dateutil
...
python scrapers/ptt_scraper.py
```
...
```
Ba-de/
├── .env                     # Email 設定（填入後生效，勿上傳 Git）
├── .env.example             # Email 設定範本
│
├── config/
...
│   └── utils.py             # 共用工具（DB 連線、設定檔讀取）
│
├── data/
│   └── campaign.db          # SQLite 資料庫（勿手動刪除）
│
├── logs/                    # 備用（Log 實際輸出至 /tmp）
└── venv/                    # Python 虛擬環境
```
...
資料庫位於 `data/campaign.db`，共四張表：

### `raw_posts`（爬蟲寫入）

| 欄位 | 說明 |
|------|------|
| `source` | 來源類型：`news` / `ptt` / `dcard` / `fb` / `gov` |
| `source_account` | Google News 為搜尋關鍵字；PTT 為看板名稱 |
| `post_id` | 平台原生 ID，作為去重依據（UNIQUE 約束） |
| `title` | 標題 |
| `content` | 內文或摘要 |
| `url` | 原文連結 |
| `published_at` | 原文發布時間（ISO 格式，含時區） |
| `scraped_at` | 爬取時間（本機時間） |
| `raw_json` | 完整原始資料，方便日後補欄位 |
...
**查看許願紀錄：**
```bash
sqlite3 data/campaign.db "SELECT created_at, name, content FROM wishes ORDER BY created_at DESC;"
```
...
```bash
# /Users/kent/Ba-de/.env
WISH_RECIPIENT_EMAIL=你的收件信箱@gmail.com
SMTP_HOST=smtp.gmail.com
SMTP_PORT=587
SMTP_USER=你的寄件Gmail@gmail.com
...
- **機制：** `StartCalendarInterval`
  - 與 `StartInterval` 的差異：電腦休眠後醒來，`StartCalendarInterval` 會補跑錯過的時間點；`StartInterval` 不會
...
for row in conn.execute(\"SELECT date(scraped_at), COUNT(*) FROM raw_posts GROUP BY date(scraped_at) ORDER BY 1 DESC LIMIT 10\"):
    print(row)
"
```
...
對 `raw_posts` 未處理的文章使用 LLM（建議 Claude API）進行：
...
結果寫入 `processed_posts` 後，Dashboard 可顯示議題趨勢與情感走向。
...
3. 手動補跑：`python scrapers/news_rss.py && python scrapers/ptt_scraper.py`
...
pip install flask feedparser pyyaml requests beautifulsoup4 python-dateutil
python db/init_db.py
cp 原備份/*.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.kent.ba-de.dashboard.plist
launchctl load ~/Library/LaunchAgents/com.kent.ba-de.scraper.plist
```
