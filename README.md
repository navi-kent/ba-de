...
┌──────────────────┐      ┌──────────────────┐        ┌─────────────┐      ┌─────────────────┐
│ news_rss.py      │─────▶│ raw_posts        │◀──讀───│ backend/    │◀────│ index.html      │
│ ptt_scraper.py   │      │ announcements    │        │ app.py:5001 │     │ news.html       │
├──────────────────┤      │ visitors         │        │             │     │ news-detail.html│
│ gov_news_scraper │─────▶│ gov_news         │        │             │     │ news-feed.html  │
└──────────────────┘      │ wishes           │        │             │     │ admin.html      │
                          │ scraper_runs     │        │             │     │ wish.html       │
後台管理員                 └──────────────────┘        └─────────────┘     └─────────────────┘
     │ 手動編輯
     ▼
 /admin
...
└── scrapers/
    ...
    ├── gov_news_scraper.py   # 八德區公所 & 戶政事務所 官網 HTML（已寫好，未加入排程）
    ├── dcard_scraper.py      # Dcard（已寫好，未加入排程）
    ├── gov_announce.py       # 政府公告 OpenData XML（已寫好，未加入排程）
...
### `gov_news`（政府官網最新消息，爬蟲寫入）

來源：八德區公所、八德區戶政事務所官網 HTML 列表（`gov_news_scraper.py`）。

| 欄位 | 型別 | 說明 |
|------|------|------|
| `source_site` | VARCHAR(30) | `bade_district`（公所）/ `bade_hro`（戶政） |
| `source_name` | TEXT | 機關中文名稱 |
| `news_id` | TEXT | 原始 URL `s=` 參數，去重依據（`UNIQUE(source_site, news_id)`） |
| `url` | TEXT | 完整文章網址 |
| `title` | TEXT | 標題 |
| `content` | TEXT | 全文（保留欄位，現階段未補爬） |
| `department` | VARCHAR(100) | 發布單位（公所才有，e.g. 民政課、社會課、農經課） |
| `published_date` | DATE | 西元日期（從民國年轉換） |
| `published_raw` | VARCHAR(20) | 原始民國年字串，e.g. `115-04-09` |
| `category` | VARCHAR(50) | 主分類（從 department / 標題關鍵字自動推導） |
| `sub_category` | VARCHAR(50) | 次分類（通常為 department 原名） |
| `tags` | TEXT[] | 關鍵詞標籤陣列（可做 GIN 索引搜尋） |
| `scraped_at` | TIMESTAMPTZ | 爬取時間 |
| `raw_json` | JSONB | 原始抓取資料 |
...
**category 對應規則（戶政）：** 從標題關鍵字推導（戶籍管理、生命事件、服務公告、人事公告、原住民事務、行政區域調整、數位服務）。
...
| 政府 OpenData | `gov_announce.py` | 桃園市政府 & 八德區公所 OpenData XML，寫入 `raw_posts` |
| 政府官網 HTML | `gov_news_scraper.py` | 八德區公所 & 戶政事務所官網列表，寫入 `gov_news`；支援增量更新 |
| Dcard | `dcard_scraper.py` | mood / trending / relationship 看板，標題/內文須含八德詞彙 |

### 未啟用（效果不穩定）

| 爬蟲 | 說明 |
|------|------|
| `fb_mbasic.py` | Facebook mbasic（結構易變、反爬蟲機制強，不穩定） |
...
# gov_news 分類統計
psql bade -U bade_user -c "SELECT source_name, category, COUNT(*) FROM gov_news GROUP BY source_name, category ORDER BY source_name, COUNT(*) DESC;"

# 手動補跑政府官網爬蟲
cd /Users/kent/Ba-de && source venv/bin/activate && python scrapers/gov_news_scraper.py
```
...
<dict>
    ...
    <array>
        ...
        <string>-c</string>
        <string>cd /Users/kent/Ba-de &amp;&amp; source venv/bin/activate &amp;&amp; python scrapers/news_rss.py &amp;&amp; python scrapers/ptt_scraper.py &amp;&amp; python scrapers/gov_announce.py &amp;&amp; python scrapers/gov_news_scraper.py &amp;&amp; python scrapers/dcard_scraper.py</string>
    </array>
